...
# ORIGINAL
# Create Dictionary
def creatRealDictionary(T,Drr,Dtheta,normalize_dict=True,device='cpu'):
    WVar = []
    Wones = torch.ones(1).to(device)
    ...
    for i in range(0,T):
        ...
        W4 = torch.mul ( torch.pow(-Drr,i) , torch.sin(i*Dtheta) )
                # double poles ... 
#         W5 = i*torch.mul(torch.pow(Drr,i) , torch.cos(i * Dtheta))
#         W6 = i*torch.mul ( torch.pow(-Drr,i) , torch.cos(i * Dtheta) )
#         W7 = i*torch.mul ( torch.pow(Drr,i) , torch.sin(i *Dtheta) )
#         W8 = i*torch.mul ( torch.pow(-Drr,i) , torch.sin(i*Dtheta) )
        
#         W = torch.cat((Wones,W1,W2,W3,W4,W5,W6,W7,W8),0)
...
def fista(D, Y, lambd,maxIter,device):

    DtD = torch.matmul(torch.t(D),D)
    L = torch.norm(DtD,2)
    linv = 1/L
    DtY = torch.matmul(torch.t(D),Y)
#     print(DtD.shape,DtY.shape)
    x_old = Variable(torch.zeros(DtD.shape[1],DtY.shape[2]).to(device), requires_grad=True)
    t = 1
    y_old = x_old
    lambd = lambd*(linv.data.cpu().numpy())
    A = Variable(torch.eye(DtD.shape[1]).to(device),requires_grad=True) - torch.mul(DtD,linv)
...
# # Create Dictionary
# def creatRealDictionary(T, rr, theta, normalize_dict=True, device="cpu"):
#     WVar = []
#     Wones = torch.ones(1).to(device)
#     for i in range(0, T):
...
#     if normalize_dict:
#         G = torch.norm(dic, p=2, dim=0)
#         idx = torch.nonzero((G == 0), as_tuple=False)
#         nG = G.clone()
#         nG[idx] = np.sqrt(T)
#         G = nG
#         dic = dic/G
...
# def fista(D, Y, lambd, max_iter, device):
#     DtD = torch.matmul(torch.t(D), D)
#     L = torch.norm(DtD, 2)
#     linv = 1/L
#     DtY = torch.matmul(torch.t(D), Y)
#     x_old = torch.zeros(DtD.shape[1], DtY.shape[2]).to(device)
#     t = 1
#     y_old = x_old
#     lambd = lambd * linv
#     A = torch.eye(DtD.shape[1]).to(device) - torch.mul(DtD, linv)
#     DtY = torch.mul(DtY, linv)
#     softshrink = nn.Softshrink(lambd.item())
